      ...
      float? batterySOCEnd = null)
    {
      ...
      if (odometerEnd.HasValue)
      {
        if (odometerEnd.Value < 0)
          return Result<bool>.Failure(Error.ValidationError("Odometer end reading cannot be negative"));
      ...
      if (customerId <= 0)
        return Result<bool>.Failure(Error.ValidationError("Customer id can not be negativ value or zero"));
